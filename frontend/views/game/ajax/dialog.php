...
<div class="text-decoration">
    <?php if ($npc->image): ?>
        <div class="clearfix">
            <img class="float-md-end mb-3 ms-md-4" src="resources/story-<?= $storyId ?>/img/<?= $npc->image ?>" alt="<?=
        $npc->name
    ?>" style="max-width: 50%;">
        <?php endif; ?>
        ...
        <p><span class="text-warning"><?= Html::encode($playerName) ?></span> &mdash; <span class="text-muted"><?=
    nl2br(Html::encode($reply->text ?? ''))
?></span></p>
        <p><span class="text-warning"><?= Html::encode($npc->name) ?></span> &mdash; <span class="text-muted"><?=
    nl2br(Html::encode($dialog->text ?? ''))
?></span></p>
        <?php if ($dialog->replies): ?>
            <ul>
                <?php foreach ($dialog->replies as $reply): ?>
                    <li><a href="#" onclick="vtt.reply(<?= $reply->id ?>); return false;"><?=
                    nl2br(Html::encode($reply->text ?? ''))
                ?></a></li>
                <?php endforeach; ?>
            </ul>
