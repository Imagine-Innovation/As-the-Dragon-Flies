...
/** @var app/models/BackgroundSkills[] $backgroundSkills */
/** @var app/models/PlayerSkill[] $playerSkills */
/** @var int $n */
    ...
    <?php if ($backgroundSkills): ?>
        ...
        <?php foreach ($backgroundSkills as $backgroundSkill): ?>
            <div class="custom-control custom-checkbox mb-2">
                ...
                <label class="custom-control-label" for="skillCheckbox-<?= $backgroundSkill->skill_id ?>"><?= $backgroundSkill->skill->name ?></label>
            </div>
        <?php endforeach; ?>
        <br>
    <?php endif; ?>
    ...
    <?php foreach ($playerSkills as $playerSkill): ?>
        <?php if (!in_array($playerSkill->skill_id, $defaultSkills)): ?>
            <div class="custom-control custom-checkbox mb-2">
                ...
                <label class="custom-control-label" for="skillCheckbox-<?= $playerSkill->skill_id ?>"><?= $playerSkill->skill->name ?></label>
            </div>
        <?php endif; ?>
    <?php endforeach; ?>
<?php else: ?>
