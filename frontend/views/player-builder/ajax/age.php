...
$currentLabel = current(array_filter($ageTable['labels'], fn($label) => $currentAge <= $label['age']))['lib'] ?? 'fine';
?>
...
<div id="displayAgeLabel" style="visibility: <?= $currentAge === 0 ? 'hidden' : 'visible' ?>">
    <h6 class="card-subtitle">Your player is <span id="playerAgeNum"><?= $currentAge ?></span>. He is <span id="playerAgeLabel"><?=
    $currentLabel
?></span></h6>
</div>
<br>
<div class="custom-control custom-range mb-2">
    <input type="range" class="form-control-range" min="<?= $min ?>" max="<?= $lifespan ?>" value="<?= $currentAge ?>" step="<?=
    $step
?>" id="ageRange"
