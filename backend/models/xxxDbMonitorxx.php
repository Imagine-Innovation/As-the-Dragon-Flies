 ...
 */
final class xxxDbMonitorxx extends ActiveRecord
{

    const MYSQL_MONITOR = "
        SELECT DIGEST_TEXT AS sql_text, ROUND(AVG_TIMER_WAIT / 1000000000) AS avg_runtime_ms, COUNT_STAR AS calls_last_hour
        FROM performance_schema.events_statements_summary_by_digest
        WHERE SCHEMA_NAME=DATABASE() AND (DIGEST_TEXT REGEXP '^(SELECT|INSERT|UPDATE)')
    ...
    public function getExplainPlan(): array
    {
        try {
            ...
            $result = Yii::$app->db
                    ->createCommand('EXPLAIN FORMAT=JSON ' . $this->sql_text)
                    ->queryOne();
        } catch (\Throwable $e) {
     ...
     * @return string[]
     */
    public function getQuerySuggestions(): array
    {
        $sql = strtolower($this->sql_text);
        $s = [];
