...
class WebSocketHandler implements MessageComponentInterface
{
    ...
    public function onOpen(ConnectionInterface $conn): void
    {
        ...
        $this->logger->log(
            "WebSocketHandler: New connection! ({$conn->remoteAddress}) assigned appClientId: {$clientId}, resourceId: {$conn->resourceId}",
            $this->clientsMap,
        );
    ...
    public function onMessage(ConnectionInterface $from, mixed $msg): void
    {
        ...
        if ($clientId === null) {
            $this->logger->log(
                "WebSocketHandler: Error - Could not determine client ID for message from resourceId: {$from->resourceId}",
                ['message' => $msg],
                'error',
            );
            $from->close();
        ...
        $this->logger->logStart(
            "WebSocketHandler: onMessage from clientId=[{$clientId}], resourceId=[{$from->resourceId}]",
            ['message_summary' => substr($message, 0, 100) . (strlen($message) > 100 ? '...' : '')],
        );
    ...
    public function onClose(ConnectionInterface $conn): void
    {
        ...
        if (!$clientId) {
            ...
            $this->logger->log(
                "WebSocketHandler: onClose - Could not determine client ID for resourceId: {$conn->resourceId}",
                null,
                'warning',
            );
            if (isset($this->clientsMap[$conn->resourceId])) {
        ...
        $this->logger->logStart(
            "WebSocketHandler: onClose for clientId=[{$clientId}], resourceId=[{$conn->resourceId}]",
        );
    ...
    public function onError(ConnectionInterface $conn, \Exception $e): void
    {
        ...
        if (!$clientId) {
            $this->logger->log(
                "WebSocketHandler: onError - Could not determine client ID for resourceId: {$conn->resourceId}. Error: "
                    . $e->getMessage(),
                $e->getTraceAsString(),
                'error',
            );
        ...
        $this->logger->logStart(
            "WebSocketHandler: onError for clientId=[{$clientId}], resourceId=[{$conn->resourceId}]",
            ['error' => $e->getMessage()],
        );
        ...
        $this->logger->log(
            "WebSocketHandler: An error occurred with client clientId=[{$clientId}]: " . $e->getMessage(),
            $e->getTraceAsString(),
            'error',
        );
